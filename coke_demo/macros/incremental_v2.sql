{% materialization incremental_v2, adapter='snowflake' -%}

  {%- set unique_key = config.get('unique_key') -%}
  {%- set predicates = config.get('predicates') -%}
  {%- set non_update_columns = config.get('non_update_columns') -%}
  {%- set matched_predicates = config.get('matched_predicates') -%}
  ...
  {%- else -%}
    ...
    {%- call statement('main') -%}
      {% if strategy == 'merge' %}
        {{ get_merge_sql_v2(target_relation, tmp_relation, unique_key, dest_columns, predicates, non_update_columns, matched_predicates) }}
